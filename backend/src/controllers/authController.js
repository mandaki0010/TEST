...
const login = async (req, res) => {
  try {
    ...
    if (!isValidPassword) {
      const attempts = User.incrementLoginAttempts(user.id);
      const maxAttempts = parseInt(process.env.MAX_LOGIN_ATTEMPTS) || 5;
      const remaining = maxAttempts - attempts;

      logger.warn(`Failed login attempt for user: ${username}, remaining attempts: ${remaining}`);

      if (remaining <= 0) {
        return res.status(423).json({ error: 'ログイン試行回数を超えました。15分後に再度お試しください' });
      }
